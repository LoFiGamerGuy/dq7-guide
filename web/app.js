...
function renderPowerPlan(plan = {}) {
  ...
  const gearRow = row => {
    ...
    const route = row.obtainable_routes?.[0], extraRoutes = Math.max((row.obtainable_routes?.length || 0) - 1, 0);
...
function renderProgress() {
  ...
  $("#equipmentReadiness").innerHTML = `<div class="callout"><strong>Validated equipment tracking enabled</strong><span>Owned recommendations can be equipped from Power up now; every write checks slot category, global copy allocation, and verified character compatibility.</span></div>${accessoryEditor}${coverage.audited_item_rows ? `<h4>Compatibility evidence</h4><p class="muted">${escapeHtml(`${coverage.verified_item_rows}/${coverage.catalog_item_rows || coverage.audited_item_rows} catalog rows verified by two independent sources · ${coverage.conflicted_item_rows} disputed · ${coverage.single_source_item_rows} single-source · ${coverage.unaudited_item_rows || 0} unaudited`)}</p>` : ""}${mechanics.length ? `<h4>Verified mechanics</h4><div class="detail-list">${mechanics.map(row => `<div><strong>${escapeHtml(row.rule_type === "slot_count" ? `${row.numeric_value} ${row.slot_name} slot${row.numeric_value === 1 ? "" : "s"}` : `Monster Heart uses ${row.numeric_value} accessory slot`)}</strong><span>${escapeHtml(row.applies_to)} · independently corroborated</span>${sourceLink({source_url: row.source_url, source_title: row.source_title, locator: row.locator})}${sourceLink({source_url: row.corroborating_source_url, source_title: row.corroborating_source_title, locator: row.corroborating_locator})}</div>`).join("")}</div>` : ""}${recommendations.length ? `<h4>Strongest-now comparison</h4><div class="detail-list">${recommendations.map(row => `<div><strong>${escapeHtml([row.character, row.slot, row.item_name].filter(Boolean).join(" · "))}</strong><span>${escapeHtml(row.comparison_status.replaceAll("_", " "))} · quantity ${escapeHtml(row.quantity_fit || "unknown")} (${escapeHtml(row.quantity_status || "unknown")}${row.quantity !== null && row.quantity !== undefined ? ` ${escapeHtml(row.quantity)}` : ""}) · ${escapeHtml(row.availability_status.replaceAll("_", " "))} · compatibility ${escapeHtml((row.compatibility_status || "unknown").replaceAll("_", " "))}</span><span>${escapeHtml(row.recommendation)}</span><span class="tag">Evidence: ${escapeHtml(row.evidence?.tier?.replaceAll("_", " ") || "audit pending")}</span>${row.equip_block_reason ? `<span>${escapeHtml(row.equip_block_reason)}</span>` : ""}${sourceLink({source_url: row.source?.url, source_title: row.source?.title, locator: row.source?.locator})}</div>`).join("")}</div><p class="muted">These are attributed checkpoint recommendations; every write still requires explicit quantity, verified compatibility, and route availability.</p>` : '<p class="muted">No sourced gear recommendation is normalized for the saved checkpoint.</p>'}`;
}
...
function renderSources(sources = state.checkpoint?.sources || []) {
  ...
  gapTarget.innerHTML = audit ? `<p class="muted">Priority gaps: ${audit.single_source} single-source · ${audit.unsupported} unsupported · ${audit.corroborated_but_unresolved} corroborated but unresolved. Separate conflict ledger: ${audit.unresolved_conflicts} unresolved. Sources: ${audit.source_freshness.over_180_days} stale · ${audit.source_freshness.unknown} retrieval date unknown.</p><div class="detail-list">${audit.gaps.map(gap => `<div><strong>${escapeHtml(gap.subject)}</strong><small class="evidence-strength">${escapeHtml(gapStrength(gap))}</small><span class="muted">Open question · ${escapeHtml(gapStatus(gap))}</span><span>${escapeHtml(gap.summary)}</span><span><b>Needed:</b> ${escapeHtml(gap.acceptance_condition)}</span>${gap.sources.map(source => `<a href="${escapeHtml(source.url)}" target="_blank" rel="noreferrer">${escapeHtml(source.title)}</a>`).join(" ") || '<span class="muted">No publishable source found.</span>'}<span class="muted">${escapeHtml(gap.freshness_status.replaceAll("_", " "))} · audited ${escapeHtml(gap.last_audited)}</span></div>`).join("")}</div>` : '<p class="empty">Evidence audit unavailable.</p>';
...
function renderRichDetail(detail, summary) {
  ...
  if (state.domain === "items") {
    ...
    const quantityLabel = item.quantity_status === "exact" ? `Exact copies: ${item.quantity}` : item.quantity_status === "at_least_one" ? "Copies: at least 1 · exact total unknown" : "Copies: unknown · not zero";
    ...
    const semantics = counterSemantics.length ? `<h4>Counter evidence</h4><div class="detail-list">${counterSemantics.map(row => `<div><strong>${escapeHtml(row.predicate.replaceAll("_", " "))}</strong><span>${escapeHtml(typeof row.value === "string" ? row.value : JSON.stringify(row.value))}</span>${sourceLink(row)}</div>`).join("")}</div>` : "";
    const conflicts = counterConflicts.length ? `<div class="uncertain-banner"><strong>Counter rule conflict — do not assume</strong><span>${escapeHtml(counterConflicts.flatMap(row => row.claims || []).map(row => typeof row.value === "string" ? row.value : JSON.stringify(row.value)).join(" versus "))}</span></div>` : "";
    const conflictSafeStrategy = detail.achievement_id === "ach_massively_minted" && counterConflicts.length
      ? '<div class="callout"><strong>Conflict-safe strategy · synthesis</strong><span>Bank 300,000 gold at once before spending it. This satisfies both published conditions; it does not resolve which counter rule the game uses.</span></div>'
      : "";
