...
function renderSources(sources = state.checkpoint?.sources || []) {
  ...
  const conflicts = $("#conflicts"); conflicts.innerHTML = state.conflicts.map(c => `<article class="conflict-item"><div class="conflict-heading"><strong>${escapeHtml(c.subject)}</strong><span class="tag">${escapeHtml(c.status || "unresolved")}</span></div><p class="muted">${escapeHtml(c.predicate)} · ${c.status === "resolved" ? "Resolution is identified below." : "No resolution is implied."}</p><div class="claim-grid">${(c.claims || []).map((claim, index) => `<section class="claim-card" aria-label="Claim ${index + 1}"><h4>Claim ${index + 1}${claim.is_resolution ? " · Resolution" : ""}</h4><p>${escapeHtml(typeof claim.value === "string" ? claim.value : JSON.stringify(claim.value))}</p><dl><dt>Scope</dt><dd>${escapeHtml(JSON.stringify(claim.scope || {}))}</dd><dt>Evidence</dt><dd>${escapeHtml(claim.confidence)} · ${escapeHtml(claim.verification_status)}</dd><dt>Freshness</dt><dd>Updated ${escapeHtml(claim.source?.updated_at || "unknown")} · Retrieved ${escapeHtml(claim.source?.retrieved_at || "unknown")}</dd></dl><a href="${escapeHtml(claim.source?.url || "#")}" target="_blank" rel="noreferrer">${escapeHtml(claim.source?.title || "Source")}</a><br><span class="muted">${escapeHtml(claim.locator || "Precise locator not stored")}</span></section>`).join("")}</div>${c.resolution_is_external ? `<section class="claim-card resolution-card" aria-label="Consensus resolution"><h4>Consensus resolution · separate claim</h4><p>${escapeHtml(typeof c.resolution.value === "string" ? c.resolution.value : JSON.stringify(c.resolution.value))}</p><dl><dt>Evidence</dt><dd>${escapeHtml(c.resolution.confidence)} · ${escapeHtml(c.resolution.verification_status)}</dd><dt>Freshness</dt><dd>Updated ${escapeHtml(c.resolution.source?.updated_at || "unknown")} · Retrieved ${escapeHtml(c.resolution.source?.retrieved_at || "unknown")}</dd></dl><a href="${escapeHtml(c.resolution.source?.url || "#")}" target="_blank" rel="noreferrer">${escapeHtml(c.resolution.source?.title || "Consensus source")}</a><br><span class="muted">${escapeHtml(c.resolution.locator || "Precise locator not stored")}</span></section>` : ""}${c.rationale ? `<p class="muted">Recorded rationale: ${escapeHtml(c.rationale)}</p>` : ""}${c.required_evidence ? `<p><strong>Needed to resolve:</strong> ${escapeHtml(c.required_evidence)}</p>` : ""}</article>`).join(""); if (!conflicts.children.length) conflicts.append(empty());
}
