...
function renderSources(sources = state.checkpoint?.sources || []) {
  ...
  const conflicts = $("#conflicts"); conflicts.innerHTML = state.conflicts.map(c => `<article class="conflict-item"><div class="conflict-heading"><strong>${escapeHtml(c.subject)}</strong><span class="tag">${escapeHtml(c.status || "unresolved")}</span></div><p class="muted">${escapeHtml(c.predicate)} · No resolution is implied.</p><div class="claim-grid">${(c.claims || []).map((claim, index) => `<section class="claim-card" aria-label="Claim ${index + 1}"><h4>Claim ${index + 1}</h4><p>${escapeHtml(typeof claim.value === "string" ? claim.value : JSON.stringify(claim.value))}</p><dl><dt>Scope</dt><dd>${escapeHtml(JSON.stringify(claim.scope || {}))}</dd><dt>Evidence</dt><dd>${escapeHtml(claim.confidence)} · ${escapeHtml(claim.verification_status)}</dd><dt>Freshness</dt><dd>Updated ${escapeHtml(claim.source?.updated_at || "unknown")} · Retrieved ${escapeHtml(claim.source?.retrieved_at || "unknown")}</dd></dl><a href="${escapeHtml(claim.source?.url || "#")}" target="_blank" rel="noreferrer">${escapeHtml(claim.source?.title || "Source")}</a><br><span class="muted">${escapeHtml(claim.locator || "Precise locator not stored")}</span></section>`).join("")}</div>${c.rationale ? `<p class="muted">Recorded rationale: ${escapeHtml(c.rationale)}</p>` : ""}</article>`).join(""); if (!conflicts.children.length) conflicts.append(empty());
}
