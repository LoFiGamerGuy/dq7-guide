...
function renderPowerPlan(plan = {}) {
  ...
  const gearRows = gear.filter(row => row.availability_status === "route_available").slice(0, 4).map(row => {
    ...
    const statLabels = { attack_bonus: "Atk", defence_bonus: "Def", agility_bonus: "Agi", magical_might_bonus: "Mag Mt", magical_mending_bonus: "Mag Mend", elemental_damage_reduction_percent: "all-element reduction", mp_absorption_percent: "MP absorption" };
    const stats = row.verified_stats || {};
    const statText = Object.entries(stats).filter(([key]) => key !== "battle_use_effect").map(([key, stat]) => ["elemental_damage_reduction_percent", "mp_absorption_percent"].includes(key) ? `${stat.value}% ${statLabels[key]}` : `${Number(stat.value) > 0 ? "+" : ""}${stat.value} ${statLabels[key]}`).join(" · ");
    const battleEffect = stats.battle_use_effect?.value;
    return `<li><strong>${escapeHtml([row.character, row.item_name].filter(Boolean).join(" · "))}</strong><span>${escapeHtml(equipped ? "Equipped" : owned ? "Owned" : "Ownership unknown")} · ${escapeHtml((row.compatibility_status || "compatibility unknown").replaceAll("_", " "))}</span>${statText ? `<span><b>Verified:</b> ${escapeHtml(statText)}</span>` : ""}${battleEffect ? `<span><b>Battle use:</b> ${escapeHtml(battleEffect)}</span>` : ""}${route ? `<span><b>Get now:</b> ${escapeHtml(route.route_label || route.location_text || route.method)}${route.is_free === 1 ? " · free" : ""}${extraRoutes ? ` · +${extraRoutes} routes` : ""}</span>` : ""}${canRecord ? `<span class="muted">Ownership tracking only · does not equip</span><button class="secondary compact-button" type="button" aria-label="Mark ${escapeHtml(row.item_name)} owned; does not equip it" data-power-item-owned="${escapeHtml(row.item_id)}">Mark owned</button>` : ""}${canEquip ? `<button class="secondary compact-button" type="button" aria-label="Equip ${escapeHtml(row.item_name)} on ${escapeHtml(row.character)}" data-power-equip-item="${escapeHtml(row.item_id)}" data-power-equip-character="${escapeHtml(row.character)}" data-power-equip-slot="${escapeHtml(row.slot)}">Equip</button>` : ""}</li>`;
  }).join("");
