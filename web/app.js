...
function renderSources(sources = state.checkpoint?.sources || []) {
  const gapStrength = gap => ({ single_source: `${gap.supporting_claim_publisher_count} publisher on partial evidence`, unsupported: "No publishable source", corroborated_but_unresolved: `${gap.supporting_claim_publisher_count} publishers on partial/negative evidence` }[gap.verification_tier] || "Evidence status unknown");
  const gapStatus = gap => ({ corroborated_inexact: "Exact observation still missing", incomplete_methodology: "Comparable benchmark still missing", single_source: "Independent corroboration missing", guide_text_conflict: "Direct UI needed to resolve conflict", no_publishable_source: "Reproducible evidence missing" }[gap.status] || gap.status.replaceAll("_", " "));
  ...
  const allStops = openStops.map((stop, index) => `<div class="sources-stop-action"><strong>${index + 1}. ${escapeHtml(stop.title)}</strong><span>${escapeHtml(stop.action)}</span></div>`).join("");
  sourcesStop.innerHTML = openStops.length ? `<strong>Active checkpoint STOP · First of ${openStops.length}</strong><span>${escapeHtml(openStops[0].title)} — ${escapeHtml(openStops[0].action)}</span>${openStops.length > 1 ? `<details class="sources-stop-list"><summary>Show all ${openStops.length} STOP actions</summary>${allStops}</details>` : ""}<a href="#walkthrough" data-view="walkthrough">Return to Now</a>` : "";
  ...
  gapTarget.innerHTML = audit ? `<p class="muted">Priority gaps: ${audit.single_source} with one publisher on partial evidence · ${audit.unsupported} unsupported · ${audit.multi_publisher_partial_evidence} with multi-publisher partial/negative evidence. Every question remains unresolved. Separate conflict ledger: ${audit.unresolved_conflicts} unresolved. Sources: ${audit.source_freshness.over_180_days} stale · ${audit.source_freshness.unknown} retrieval date unknown.</p><div class="evidence-gap-list">${gapCards}</div>` : '<p class="empty">Evidence audit unavailable.</p>';
