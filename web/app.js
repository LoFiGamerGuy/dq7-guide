...
function renderProgress() {
  ...
  $("#equipmentReadiness").innerHTML = `<div class="uncertain-banner"><strong>Equipment editing unavailable</strong><span>${escapeHtml((equipment.gaps || ["Character/item compatibility is not normalized."])[0])}</span></div>${mechanics.length ? `<h4>Verified mechanics</h4><div class="detail-list">${mechanics.map(row => `<div><strong>${escapeHtml(row.rule_type === "slot_count" ? `${row.numeric_value} accessory slots` : `Monster Heart uses ${row.numeric_value} accessory slot`)}</strong><span>${escapeHtml(row.applies_to)} · independently corroborated</span>${sourceLink({source_url: row.source_url, source_title: row.source_title, locator: row.locator})}${sourceLink({source_url: row.corroborating_source_url, source_title: row.corroborating_source_title, locator: row.corroborating_locator})}</div>`).join("")}</div>` : ""}${recommendations.length ? `<h4>Strongest-now comparison</h4><div class="detail-list">${recommendations.map(row => `<div><strong>${escapeHtml([row.character, row.slot, row.item_name].filter(Boolean).join(" · "))}</strong><span>${escapeHtml(row.comparison_status.replaceAll("_", " "))} · ownership ${escapeHtml(row.ownership_status)} · ${escapeHtml(row.availability_status.replaceAll("_", " "))}</span><span>${escapeHtml(row.recommendation)}</span>${sourceLink({source_url: row.source?.url, source_title: row.source?.title, locator: row.source?.locator})}</div>`).join("")}</div><p class="muted">These are attributed checkpoint recommendations, not a complete equipability matrix.</p>` : '<p class="muted">No sourced gear recommendation is normalized for the saved checkpoint.</p>'}`;
}
...
async function updateProgress(payload) {
  setStatus("Saving…");
  const resources = { item: "items", tablet: "tablets", achievement: "achievements", missable: "missables" };
