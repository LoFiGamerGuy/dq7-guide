...
function renderSources(sources = state.checkpoint?.sources || []) {
  ...
  const gapCards = audit?.gaps.map(gap => `<details class="evidence-gap-card"><summary><span class="evidence-gap-heading"><strong>${escapeHtml(gap.subject)}</strong><small class="evidence-strength">${escapeHtml(gapStrength(gap))}</small></span><span class="muted">Open question · ${escapeHtml(gapStatus(gap))}</span></summary><div class="evidence-gap-body"><span>${escapeHtml(gap.summary)}</span><span><b>Needed:</b> ${escapeHtml(gap.acceptance_condition)}</span>${gap.sources.map(source => `<a href="${escapeHtml(source.url)}" target="_blank" rel="noreferrer">${escapeHtml(source.title)}</a>`).join(" ") || '<span class="muted">No publishable source found.</span>'}<span class="muted">${escapeHtml(gap.freshness_status.replaceAll("_", " "))} · audited ${escapeHtml(gap.last_audited)}</span></div></details>`).join("");
