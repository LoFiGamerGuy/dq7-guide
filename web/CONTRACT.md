...
`GET /api/moonlighting` (also embedded in vocation list/detail responses) returns the direct unlock and mechanics claims with full provenance. cp012 after Aishe is the normalized earliest checkpoint. Game8 names Alltrades Abbey for activation while RPG Site names the Shrine of Mysteries, so both unlock claims remain visible with `venue_status: "conflicting_sources"`; the browser does not resolve them. The safe walkthrough action is to follow Jacqui's Career Sphere prompt, try the Shrine event, and use Alltrades Abbey as the explicit fallback if Moonlighting remains locked. Resolution requires a current-version capture or continuous video showing both the post-Aishe message and activation interaction with the venue name visible. Published behavior covers two equipped vocations, the Career Sphere tab, simultaneous learning, and access to both sets of skills/stat bonuses. Unpublished point splitting, complete legal-pair restrictions, and skill retention outside the equipped pair remain explicit unknowns. Pair suggestions remain recommendations.
