...
runtime_dir="$repo_dir/.guide-runtime"
...
start_server() {
  ...
  mkdir -p "$runtime_dir"; : > "$log_file"; cd "$repo_dir"
  if [ "${1:-}" = "rotate" ]; then
    nohup python3 -u scripts/guide_server.py --lan --pairing-file "$runtime_dir/phone-pairing-token" --rotate-pairing >"$log_file" 2>&1 &
  else
    nohup python3 -u scripts/guide_server.py --lan --pairing-file "$runtime_dir/phone-pairing-token" >"$log_file" 2>&1 &
  fi
  server_pid=$!; printf '%s\n' "$server_pid" > "$pid_file"; attempts=0
...
stop_server() {
  ...
  rm -f "$pid_file"; echo "DQ7 phone guide stopped. The pairing URL is now invalid."
}
...
case "$command_name" in
  start) start_server ;; stop) stop_server ;; restart) stop_server; start_server ;; rotate) stop_server; start_server rotate ;; status) show_status ;;
  ...
  *) echo "Usage: $0 {start|stop|restart|rotate|status|foreground|install-shortcut|remove-shortcut}" >&2; exit 2 ;;
esac
