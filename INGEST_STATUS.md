...
The phone cache namespace advances to v9 with publisher-counted, locator-complete evidence-gap cards. Evidence tiers and badges now derive from distinct publishers rather than page count; the Blue Button row correctly reports two publishers across three pages. Expanded cards pair every atomic supporting claim with its source URL and precise locator, while separately labelling additional audited pages. Activation removes old shell/data caches; paired authenticated API responses remain network-only.
