...
| Equipment | 110 ready-for-play advice rows; two accessory mechanics corroborated; 311/312 canonical equipment rows audited, including ordinary accessories and Hearts | 299 rows have two-publisher agreement; 10 conflicts, 2 single-source rows, and the legacy `Meowgiican Heart` duplicate identity remain unnormalized | Resolve residual rows; corroborate duplicate/stack behavior and one-each non-accessory slot counts before enabling writes |
