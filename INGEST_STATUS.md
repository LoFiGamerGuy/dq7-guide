...
|---|---:|---|---|
| Source registry | 717 registered pages; all retrieval dates are currently within 180 days | High metadata / page publication freshness is still mixed | Add official and in-game evidence sources only where they close an explicit gap; refresh retrievals before the 180-day audit threshold |
...
| Heroic Hoarder items | 353/353 required identities / 748 acquisition paths across 355 shared items; all required items have routes; Fire Blade now has its finite Burnmount chest alternative to Lucky Panel | High for identities and route existence; former `container_unspecified` rows are closed, but some older generic `source_checked` finite rows remain area-only; Stellar Fan is resolved from direct English UI while Stella Fan remains a sourced losing alias | Continue exact-container refinement without treating a status-tag audit as proof that every route is individually exact |
...
- **Items:** no `container_unspecified` acquisition row remains. AppMedia explicitly assigns Ruby of Protection to Faraday Castle Present's throne-bedroom left drawer and 200 gold to the right; Altema's independently published labelled map matches that orientation. Direct English PC gameplay resolves the other nine former residuals: six Yet Another World postgame chests, L'Arca's isolated-north Pretty Betsy gold sparkle, Temple Palace's Gold Bracer chest, and Hubble's Silk Tuxedo dresser. Platform/patch scope remains unstated for the Ruby pages, and the footage's `Wind Tower - Level 3` versus Neoseeker's `NW Tower 3F` labels remain preserved.
...
- **Provenance:** required source-bearing tables pass locator and foreign-key validation. All 717 registered sources currently have retrieval dates within 180 days, but retrieval recency does not establish publication/patch freshness; indexed/snippet evidence is not promoted to canonical fact.
...
room beyond the east-side disappearing root. RPG Site and Game8 Japan directly
identify the sparkle, while Neoseeker independently corroborates the exact
navigation. This establishes one finite pickup, not player ownership or a
repeatable source.

The Rippled Rapier route is now a two-publisher exact chest rather than a generic
Wetlock acquisition: after Highendreigh Tower, report to Professor Wade and use
the west raft route to his Present underground storeroom, then open the chest
beside the Red Fragment. Gamers-High and AppMedia independently publish the route.
This establishes the finite pickup and story gate, not player ownership.
...
Heart at B3 south, and Divine Bustier at B4 south. Gamers-High and AppMedia agree
on all three floor positions. These remain finite one-copy routes; no ownership,
repeatability, or duplicate-legality inference is added.
...
Tower chest to the 5F chest beside the Red Fragment after descending from 6F.
RPG Site and Neoseeker independently publish both exact routes. Each remains a
finite one-copy pickup; the evidence does not imply ownership, duplicate legality,
repeatability, or a numeric rate. The existing cp003 phone obligation already names
these containers, and item/Heart detail now uses the same normalized exact routes.

The Rampage Road roster batch replaces all 34 generic Arena routes with exact
road/round locations. Gamers-High and RPG Site independently match the Testy,
Simmering, Fuming, and Furious starting groups except one member. GameWith and
Kita-Tenkara independently establish Rampaging Miry Hand and Miry Mudraker as
Furious Round 1 summons from Orgodemir's second form, not starting opponents.
Gamers-High explicitly lists Rampaging Sunken Spirit in Simmering Round 3 while
RPG Site omits it; the route remains single-source and the roster pair is the
one visible unresolved conflict pending direct encounter UI or another complete
current-version roster.

The hidden-broadness audit adds Fire Blade's finite, free Burnmount Present Level 5
Ultimate-Key chest from AppMedia and GameWith, resolving RPG Site's isolated Lucky
Panel-exclusive qualifier without removing its valid Version 3 Rank 4 panel route.
It also replaces three generic Cathedral of Blight acquisitions with the exact
current-version elemental-vault interactions: Water Amulet for Okeanos Sword in
Whitewater Cave, Fire Amulet for Pyros Helm in Hellfire Hollow, and Earth Amulet
for Gaia Armour in Chasm of Chaos. Game8 Japan and Neoseeker independently support
each route. The audit also corrects the documentation boundary: absence of the
former `container_unspecified` tag is not proof that every older generic
`source_checked` finite route has an individually identified container.

The exact Ruby drawer batch closes the final grouped-container acquisition
residual. AppMedia's current-version Faraday Present table explicitly maps Ruby
of Protection to the throne-bedroom left drawer and 200 gold to the right.
Altema independently corroborates both individual members on its labelled
Present throne-room/2F map. Platform, patch, and region are unstated; the older
RPG Site grouped claim and Lord Fenton observation limit remain preserved rather
than rewritten. Search now returns the two exact claims instead of an open gap.

The farming-rate residual is now source-backed without promoting incomplete
numbers. NoobFeed reports a Wetlock setup and approximate Strength Seed yield;
Vortex reports an Almighty rematch build and approximate loop/hour figures. The
atomic claims enumerate each missing control. Neither names the full platform,
patch, difficulty, build, attempt, and measurement scope required for comparison,
so no rate enters the farming tables or phone power advice.
...
all six open gaps, including `farming EXP rate` and
`achievement counter persistence`;
the query layer fails loudly if that vocabulary and the curated queue diverge.
The follow-up natural-language regression covers all six open rows. The resolved
Ruby drawer now returns exact sourced claims. Common question words no longer suppress a curated unknown, and a
...
gap matching, allowing `best EXP farming rate` to lead with that preserved unknown.
...
routes and five retain explicit Heart gates. Metal Slime and Gold Golem now use a
structured first-control DLC gate, while missing entitlement keeps current
availability unknown rather than exposing future/unowned content.
...
isolated claim of chance drops from respawning Trolls remains visible as losing,
low-confidence evidence; neither the sparkle nor any second copy is inferred to
respawn.

The counter-observability pass registers the current-version Detailed Records menu's
separate visible battle-win, monsters-defeated, field-attack, quick-win, and
metal-family-defeat counters. This does not infer overlap or units. The phone Sources
residual now specifies the raw before/after multi-enemy, metal-family, and save-state
capture protocol needed to resolve the four remaining telemetry questions.
...
The safe-advancement pass adds a conservative checkpoint readiness summary over explicit STOP, required-obligation, and completion-ledger state. STOP warnings retain priority and are not conflated with ledger review. After those blockers clear, the API, phone, and checkpoint CLI enumerate unrecorded available/backtrack Medals, checkpoint Tablet Fragments, finite Heroic Hoarder items, due achievements, and checkpoint missables; the explicit advance action remains disabled until those ledgers are recorded or reviewed. Each nonzero phone gap is a large navigation-only control that opens and focuses its exact collapsed ledger, including sticky-header-safe scrolling in narrow layouts. The sourced prose exit condition always requires player confirmation and is never inferred.

The phone cache namespace advances to v10 with atomic-claim-publisher-counted evidence-gap cards. Evidence tiers and badges now derive only from distinct publishers behind locator-linked supporting claims, not page count or additional unlinked audited pages. The repeatable-Heart row therefore reports five claim publishers while retaining all eight audited pages in expanded detail. Activation removes old shell/data caches; paired authenticated API responses remain network-only.

The missable-result follow-up replaces the phone's completed/unknown checkbox with an explicit unknown/completed/missed selector in both checkpoint and registry detail views. Recording missed requires confirmation, is reversible through the same control and undo action, and activates the 100%-recovery-required advancement gate. The API enforces mutual exclusion between completed and missed state; neither result is inferred.
...
- **Curated research queue:** 6 items: 1 single-source, 0 unsupported, and 5
...
- **Freshness:** all 717 source records currently have retrieval dates within 180
...
The early Heart-container batch converts Little Devil Heart and Healslime Heart
from broad `other` acquisitions to exact finite chest routes. Both use the
already registered current-version NightlyGamingBinge Heart guide with atomic
location claims and precise per-Heart locators. This reduces the explicit
container residual from 28 to 26 without asserting repeatability, a second copy,
or a numeric drop rate.

The later Heart-container batch resolves four more residuals from the same
registered current-version guide: Hypothermion Heart is in the southeastern
Level 4 Malign Shrine chest; Wight Watchman Heart is the Level 5 Beacon eastern-
platform gold pickup during The Skystone Comes Crashing Down; Goodybag Heart is
in a Below Decks treasure-room chest aboard the Sea Dragon; and Drakulard Heart
is in the Another World Level 3 chest behind the Ultimate-Key gate. Atomic
claims retain every route qualifier. The explicit container residual falls from
26 to 22; none of these finite routes is promoted to a repeatable source.

The Hubble container-semantics batch registers NightlyGamingBinge's current-
version Magic Key chest guide and resolves Princess's Robe to locked chest #7 in
the Water Tower on Level 1, including its Magic Key prerequisite. Neoseeker's
continuous Hubble walkthrough corrects Silk Tuxedo from a chest to the NW Tower
3F king-room dresser pair. Because the page groups Silk Tuxedo with Bow Tie and
does not identify which dresser holds which item, that member-level unknown is
preserved. The residual falls from 22 to 21 rather than falsely claiming both
containers exact.

The Great Helm follow-up uses Game8's current-version postgame route to replace
the broad Another World floor range with the exact north chest on 4F, reached by
the left stairs from 3F. Direct English video later resolves Sun Crown and
Ruinous Shield to their individual chests; neither remains in the current
container residual.

The early grouped-container follow-up resolves Fishnet Stockings to the east
closet in Allblades Arena's north room and Mermaid Moon to the Wetlock Treasure
Room chest after completing Hardlypool and asking the guard to open the gate.
Ruby of Protection is narrowed to the Faraday Castle throne-bedroom drawers,
