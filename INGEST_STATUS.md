...
|---|---:|---|---|
| Source registry | 682 registered pages; all retrieval dates are currently within 180 days | High metadata / page publication freshness is still mixed | Add official and in-game evidence sources only where they close an explicit gap; refresh retrievals before the 180-day audit threshold |
...
| Heroic Hoarder items | 353/353 required identities / 747 acquisition paths across 355 shared items; all required items have routes; direct finite pickups now provide free alternatives for panel-listed equipment throughout the early and midgame in addition to normalized monster-drop alternatives | High for identities and explicit routes; 15 finite routes retain explicit container-member uncertainty (12 broad/grouped other routes and 3 chest routes); Stellar Fan is resolved from direct English UI while Stella Fan remains a sourced losing alias | Expand alternate free routes only where direct current-version evidence adds practical value |
...
| Conflicts | Automatic exact-scope detection active; all 413 claim-pairs are resolved. Massively Minted is adjudicated to lifetime total gold acquired by four-publisher agreement; the simultaneous-balance claim remains visible as losing evidence. Troll Heart's field-sparkle route overrides a contradicted respawning-drop claim; Stellar Fan is resolved by direct English UI, and fourteen compatibility pairs use linked two-publisher consensus claims | High; every losing claim and rationale remains visible | Preserve conflict visibility and reopen only if stronger same-scope evidence disagrees |
...
- conflicts: 413 total / 0 unresolved claim-pairs / 0 unresolved fact scopes
...
- **Items:** the priority finite-container video queue is resolved, but 15 acquisition rows still deliberately carry `container_unspecified`: 12 broad or grouped-container `other` routes and three chest routes. These are not silently counted as exact containers; they require semantic reclassification or direct member-level evidence. Direct English item-result footage resolves Dragon Shield, Pirate's Hat, Silk Robe, Steel Helmet, and Knuckledusters to individual containers. A current-version PS5 video with Italian UI resolves Faraday Strength Ring to the lower/southern east-wall bedroom drawer, with Single Phial in the adjacent upper/northern drawer; the English localization is not inferred. Present Poolside Cave Fur Cape is resolved to the lone chest in the B2 section-1 northeast terminal alcove; Coagulant is corrected from Hubble Castle to the lower-roof barrel of Hubble Past's western Inquisitory. NightlyGamingBinge's current-version Heart guide resolves six Heart routes to exact containers or pickups: Little Devil, Healslime, Hypothermion, Wight Watchman, Goodybag, and Drakulard. Its Magic Key guide resolves Princess's Robe to Hubble Castle Water Tower Level 1's locked chest. Neoseeker corrects Silk Tuxedo from a chest to the NW Tower 3F dresser pair, but which individual dresser contains it remains unknown. Game8's current-version postgame route resolves Great Helm to the north chest on Another World 4F. RPG Site resolves Fishnet Stockings to the east closet in Allblades Arena's north room, while Game8 resolves Mermaid Moon to the Wetlock Treasure Room chest after the Hardlypool gate. Ruby of Protection is narrowed to the throne-bedroom drawer group but retains its unknown individual drawer. Game8 further proves that Day Off Dress belongs to the three chests behind Another World's Level 3 jail cell and Goddess Ring to the two chests by Yet Another World's southwest Tower lever; the individual chest in each group remains unknown. Ruinous Shield is the other dungeon-level chest residual; Sun Crown is now exact. The three formerly untyped shop-like rows have direct prices and typed shop inventory records.
...
- **Provenance:** required source-bearing tables pass locator and foreign-key validation. All 682 registered sources currently have retrieval dates within 180 days, but retrieval recency does not establish publication/patch freshness; indexed/snippet evidence is not promoted to canonical fact.
...
the left stairs from 3F. Sun Crown was later resolved by direct English video; Ruinous Shield remains a chest-
labelled residuals because the checked current-version pages establish their
dungeons but not a member-level chest locator. The total residual falls from 21
to 20 without inferring either remaining chest.
