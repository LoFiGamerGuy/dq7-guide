...
| Vocation prerequisites | 10 rule groups / 27 prerequisite edges with per-edge locators; vocation detail now exposes sourced direct unlock rules and explicit-state party progress; Luminary has a complete 25/35/40/65/75/110/130 two-source rank-cost ladder | High for direct paths and Luminary costs; other numeric mastery costs remain unpublished | Add each remaining numeric ladder only after cell-level two-source comparison |
...
| Lucky Panel | 14 normalized pools / 302 reward paths; all standard matrices are normalized; three attempts per in-game day and the inn reset are independently corroborated | High for normalized rows and attempt/reset rule; `Shell Shield` remains the sole exact-name gap. Version 1 Rank 2 retains one legacy Slime Earring row absent from the current table; entry cost and probabilities remain unknown | Verify Shell Shield, the legacy row, entry cost, and probabilities if directly published |
| Equipment | 110 ready-for-play gear, boss, grind, vocation, and tactical rows across cp001–cp033; two accessory slots and one-slot Monster Heart consumption independently corroborated | High for the two mechanics; Medium/High for attributed advice | Resolve duplicate behavior, other slot counts, and the conflicted character/item matrix before enabling writes |
...
| Monster Hearts | 46/46 normalized Hearts with sourced effects; 41/46 surface shared-item acquisition routes; Dragonlord, Malroth, and Zoma have independently corroborated DLC Arena thresholds and cp020 gates | High for effects and published routes; numeric drop rates and repeatable Heart routes remain unknown | Resolve a genuinely repeatable Heart route only from explicit rematch/respawn evidence |
...
3. Verify Shell Shield, the legacy Version 1 Rank 2 Slime Earring row, and Lucky Panel entry cost/probabilities only from direct current-version evidence.
