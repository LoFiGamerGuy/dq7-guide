...
The phone cache namespace advances to v8 with locator-complete evidence-gap cards. Expanded cards pair every atomic supporting claim with its source URL and precise locator, while separately labelling additional audited pages that are not atomic claim links. Activation removes old shell/data caches; paired authenticated API responses remain network-only.
