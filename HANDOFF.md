...
The API and CLI are regression-checked across all 46 Hearts for identical checkpoint gates, current/later/unknown state, and DLC entitlement handling. API list rows whose gate comes from an item route now also carry that route's exact source, locator, and explanatory note; this does not promote the route to repeatable.
...
Phone-cache handoff: shell and API caches use the v10 namespace. The activating worker removes older namespaces, and `controllerchange` reloads once, so the actionable ledger controls and atomic-claim-publisher-counted evidence cards replace stale assets on already-paired phones. Paired API responses remain excluded from caching.
...
HttpOnly-cookie sessions; cache v10 purges older snapshots. Rotating the private
