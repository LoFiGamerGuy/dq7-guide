...
Sources-view handoff: all six curated unknowns start collapsed and show locator-linked evidence tiers. Natural questions ignore common question words and normalize only an explicit set of plural/phrasing variants; the resolved Ruby query returns its sourced exact-location claim, while questions such as “Can I farm Monster Hearts repeatedly?” still return the preserved unknown before generic guide text. A checkpoint STOP remains visible above Sources content, and rendered portrait/landscape tests enforce the six-card queue, 4/1/1 tier split, large evidence links, and overflow safety.
