...
Slime Heart and Golem Heart now resolve to exact two-source early containers:
Rainbow Mines Past B5's only northeast chest and The Tower Past 5F's chest beside
the Red Fragment. Both remain finite one-copy routes and player ownership stays
unknown until explicitly reported.
Goon Heart and Very Devil Heart likewise resolve to exact two-source finite chests:
Dungeon of Descent Past B2's south branch and Falls Hollow Past 1F's second-room
north chest beside the staircase. These route refinements do not establish extra
copies, ownership, or repeatability.
Cathedral of Blight routes now identify Hela's Armour at the 3F east chest,
Orichalcum Fangs at the 4F north chest, and Sword of Ruin at the 1F central-throne
gold sparkle. The Sword route is a ground pickup, correcting the older generic
chest label; all three are independently corroborated by RPG Site and Gamers-High.
Headsman's Axe is independently pinned by Gamers-High and Neoseeker to the B2
small-room chest behind the left-hand door.
Hidden Pyramid routes now identify Double-Edged Sword at the B1 northwest chest,
Skeleton Swordsman Heart at the B3 south chest, and Divine Bustier at the B4 south
chest. Gamers-High and AppMedia independently corroborate all three; each remains
a finite route with player ownership unknown.
Rippled Rapier now resolves to Professor Wade's underground storeroom chest beside
the Red Fragment in Wetlock Present, reached by the west raft route after clearing
Highendreigh Tower and reporting back. Gamers-High and AppMedia independently
corroborate the exact container and story gate.
Demon Spear now resolves to the gold/orange sparkle at the back-left of Nottagen
Cavern Past B1 screen 2, beyond the east-side disappearing root. RPG Site and
...
Duplicate-Heart audit: current evidence permits two-copy ownership for Healslime,
Bodkin Archer, and Hammerhood Hearts through normal plus Japanese retailer-bonus
routes. It does not prove simultaneous duplicate equipping or effect stacking,
and bonus-code availability is unknown. Keep the editor's duplicate-Heart gate
closed and do not expose those promotional copies as currently obtainable.
...
Phone-cache handoff: shell and API caches use the v23 namespace. The activating worker removes older namespaces, and `controllerchange` reloads once, so evidence-safe strongest-gear coverage, prioritized residual evidence labels, full checkpoint advice, vocation recommendations, boss preparation, Heart routes, persistent STOP context, and corrected reconnect state reach already-paired phones. Reconnect banners use a single cancellable timer and hide only while the host remains reachable with fresh data. Paired API responses remain excluded from caching.
Sources-view handoff: all six curated unknowns start collapsed and show locator-linked evidence tiers. Natural questions ignore common question words and normalize only an explicit set of plural/phrasing variants; the resolved Ruby query returns its sourced exact-location claim, while questions such as “Can I farm Monster Hearts repeatedly?” still return the preserved unknown before generic guide text. A checkpoint STOP remains visible above Sources content, and rendered portrait/landscape tests enforce the six-card queue, 5 corroborated / 1 single-source / 0 unsupported tier split, large evidence links, and overflow safety. The farming-rate row cites NoobFeed and Vortex timed observations only to document missing controls; their numbers are not normalized or compared.
The phone now labels those badges as publishers behind partial/negative evidence,
never as corroboration of the missing proposition, and sorts them by the maintained
research priority. When several checkpoint STOP actions are open, Sources says
`First of N` and exposes a large-target expandable list of every STOP action.
...
HttpOnly-cookie sessions; cache v19 purges older snapshots. Rotating the private
...
The current-version walkthrough, map, and video audits close the original 31-row
priority finite-container queue and the broader semantic residual; no explicit
`container_unspecified` row remains. Direct English
...
several individual containers; the current residual is the one-row set summarized
...
mountain ledge reached through the south exit, and Healslime Heart is in a
Grotta del Sigillo Level 2 chest. Both remain finite routes; the evidence does
not establish repeatability, duplicate legality, or a numeric drop rate.

The same direct guide closes four later Heart-route residuals: Hypothermion is
the southeastern Level 4 Malign Shrine chest, Wight Watchman is the Level 5
Beacon eastern-platform gold pickup during The Skystone Comes Crashing Down,
Goodybag is in the Sea Dragon Below Decks treasure room, and Drakulard is in
the Another World Level 3 chest behind the Ultimate-Key gate. Each remains a
finite one-copy route; repeatability and duplicate behavior are still unknown.
...
subsequent direct evidence reduced the current total to one.

Game8's postgame route resolves Great Helm to the north chest on Another World
4F, reached by taking the left stairs from 3F. Direct English videos later
resolve Sun Crown and all three former chest-labelled residuals: Day Off Dress is
the center jail-cell chest, Goddess Ring is the lone chest in the Tower's large
western monster room, and Ruinous Shield is the west chest of its Underground
Level 1 pair. The same footage resolves Super Seed of Magic and Gigant Armour to
the west and east chests, respectively, of the Interior north-wall pair.
...
after the Hardlypool gate. That batch narrowed Ruby of Protection only to the
Faraday Castle throne-bedroom drawer group; the later AppMedia/Altema batch above
resolves its individual left drawer without rewriting the older grouped claim.
