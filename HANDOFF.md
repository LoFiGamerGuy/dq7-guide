...
Sources-view handoff: all seven curated unknowns start collapsed and show locator-linked evidence tiers. Natural questions now ignore common question words and normalize only an explicit set of plural/phrasing variants, so queries such as “Which drawer has the Ruby of Protection?” and “Can I farm Monster Hearts repeatedly?” return the unresolved record before generic guide text. A checkpoint STOP remains visible above Sources content, and rendered portrait/landscape tests enforce the seven-card queue, 5/1/1 tier split, large evidence links, and overflow safety.
...
the sole explicit `container_unspecified` row summarized above. Direct English
...
after the Hardlypool gate. Ruby of Protection is narrowed to the Faraday Castle
throne-bedroom drawer group, but its individual drawer remains unknown because
the source groups it with 200 gold.
