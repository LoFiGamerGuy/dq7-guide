...
Phone-cache handoff: shell and API caches use the v22 namespace. The activating worker removes older namespaces, and `controllerchange` reloads once, so evidence-safe strongest-gear coverage, prioritized residual evidence labels, full checkpoint advice, vocation recommendations, boss preparation, Heart routes, persistent STOP context, and corrected reconnect state reach already-paired phones. Paired API responses remain excluded from caching.
