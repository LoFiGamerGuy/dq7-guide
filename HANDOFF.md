...
Phone-cache handoff: shell and API caches use the v18 namespace. The activating worker removes older namespaces, and `controllerchange` reloads once, so evidence-safe strongest-gear coverage, full checkpoint advice, vocation recommendations, boss preparation, Heart routes, and corrected reconnect state reach already-paired phones. Paired API responses remain excluded from caching.
...
HttpOnly-cookie sessions; cache v18 purges older snapshots. Rotating the private
