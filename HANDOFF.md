...
Current reproducible headline counts are 358 sources, 354 items (353 in the registered Heroic Hoarder matrix) with 704 acquisition paths, 333 monsters with 324 checkpoint-gated encounters and 180 verified drops, all 46 Monster Heart identities/effects with linked acquisition routes for 41, 18 fixed Seed effects and one repeatable reward rule, 10 checkpoint-gated farms with separate fact/strategy provenance, 7 sourced missable records (6 exact and 1 unresolved cutoff), 14 Lucky Panel pools with 288 rewards, 100 Mini Medals, 71 tablet fragments, 61 achievements, and 26 complete vocation skill/perk ladders. Meowgician Heart now has a direct finite Vicious Meowgician route at cp005; the five remaining unlinked Hearts are explicitly DLC-scoped and await non-Heroic/DLC availability normalization. Three dedicated current-version item pages resolve Slime Earrings, Magic Vestment, and Faerie Foil aliases without changing canonical IDs; other Lucky Panel name gaps remain explicit. Direct current-version evidence adds finite free alternatives for Hairband, Rabbit Ears, Coagulant, Pretty Betsy, and Prayer Ring while retaining unknown exact containers. All directly published Lucky Panel standard-rank matrices are normalized with exact-name gaps retained. All 33 walkthrough checkpoints have direct RPG Site section-range locators; their `seed_partial` status remains because provenance completeness does not mean the guide content and optimization layers are complete. The browser includes a searchable source/freshness registry and symmetric conflict evidence. `INGEST_STATUS.md` remains the authoritative detailed ledger.
