...
Phone-cache handoff: shell and API caches use the v13 namespace. The activating worker removes older namespaces, and `controllerchange` reloads once, so complete vocation-recommendation links, atomic boss-prep evidence, structured Heart-route evidence, and prior phone improvements replace stale assets on already-paired phones. Paired API responses remain excluded from caching.
...
HttpOnly-cookie sessions; cache v13 purges older snapshots. Rotating the private
