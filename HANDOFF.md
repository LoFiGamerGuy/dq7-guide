...
Phone-cache handoff: shell and API caches use the v6 namespace. The activating worker removes older namespaces, and `controllerchange` reloads once, so the actionable ledger controls and their response shape replace stale v5 assets on already-paired phones. Paired API responses remain excluded from caching.
