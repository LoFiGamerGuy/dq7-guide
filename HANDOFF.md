...
Phone-cache handoff: shell and API caches use the v9 namespace. The activating worker removes older namespaces, and `controllerchange` reloads once, so the actionable ledger controls and publisher-counted, locator-complete evidence cards replace stale assets on already-paired phones. Paired API responses remain excluded from caching.
...
HttpOnly-cookie sessions; cache v9 purges older snapshots. Rotating the private
