...
class MobileUiContractTests(unittest.TestCase):
    ...
    def test_phone_companion_controls_and_safe_area_support_are_present(self):
        ...
        self.assertIn("function adviceEvidenceLinks(row)", js)
        self.assertIn("row.evidence?.claims", js)
        self.assertIn("When, tradeoff & evidence · ${escapeHtml(evidenceLabel)}", js)
        ...
        self.assertIn('corroborated_but_unresolved: `${gap.supporting_claim_publisher_count} publishers on partial/negative evidence`', js)
        self.assertIn('incomplete_methodology: "Comparable benchmark still missing"', js)
        self.assertIn('single_source: `${gap.supporting_claim_publisher_count} publisher on partial evidence`', js)
        ...
        self.assertIn('class="evidence-gap-card"', js)
        self.assertIn('<summary><span class="evidence-gap-heading">', js)
        self.assertNotIn('<div><strong>${escapeHtml(gap.subject)}</strong>', js)
        self.assertIn('.evidence-gap-card > summary { min-height: 3rem', css)
        self.assertIn('.evidence-gap-card > summary { grid-template-columns: 1fr', css)
        self.assertIn("Partial/negative evidence locators:", js)
        self.assertNotIn("Supporting claim locators:", js)
        self.assertIn("escapeHtml(claim.locator)", js)
        self.assertIn("claim.source?.url", js)
        self.assertIn("Additional audited pages:", js)
        self.assertIn('id="sourcesStop"', html)
        self.assertIn("Active checkpoint STOP", js)
        self.assertIn("First of ${openStops.length}", js)
        ...
        self.assertIn(".sources-stop-list > summary { min-height: 2.75rem", css)
        self.assertIn(".evidence-links a { min-height: 2.75rem", css)
        self.assertIn(".evidence-claim a, .evidence-claim small, .evidence-links a { overflow-wrap: anywhere; }", css)
    ...
    def test_active_play_writes_are_guarded_reversible_and_context_preserving(self):
        ...
        self.assertIn('aria-describedby="safeCondition advanceReason advanceLedgerGaps"', html)
        self.assertIn('id="advanceLedgerGaps"', html)
        self.assertIn('completion_ledgers_open: "Ledger review"', js)
        self.assertIn('completion_failed: "100% recovery needed"', js)
        self.assertIn('data-missable-status=', js)
        self.assertIn("Missed · recovery needed", js)
        self.assertIn("Record this missable as missed? 100% completion will require recovery", js)
        self.assertIn('JSON.stringify({ status })', js)
        self.assertNotIn('data-missable-id=', js)
        ...
        self.assertIn('selected: true, intent: "set"', js)
        self.assertIn('selected: true, intent: "advance"', js)
