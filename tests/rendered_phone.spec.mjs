...
for (const viewport of [
  ...
  test(`${viewport.name} phone workflow renders without obstruction`, async ({ page, request }) => {
    ...
    await expect(page.locator("#evidenceGaps")).toContainText("5 with multi-publisher partial/negative evidence");
    await expect(page.locator("#evidenceGaps")).toContainText("Every question remains unresolved");
    ...
    await expect(page.locator("#evidenceGaps")).toContainText("1 with one publisher on partial evidence");
    ...
    await expect(gapCards.nth(0)).toContainText("Repeatable Monster Heart routes");
    await expect(gapCards.nth(1)).toContainText("Remaining cumulative achievement semantics");
    await expect(gapCards.nth(2)).toContainText("Remaining same-item accessory and Heart duplicate stacking");
