...
| Phone access | Private persistent pairing URL; normal restart preserves bookmark; explicit rotation revokes it; a server marker keeps header- and cookie-authenticated API data network-only, and cache v9 purges older snapshots | Ready on trusted LAN |
